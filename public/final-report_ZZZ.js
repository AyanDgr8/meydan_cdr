...
const state = {
  ...
  clustered: {
    ...
    totalPages: 0,
    totalRecords: 0,
    summary: { Campaign: 0, Inbound: 0, Outbound: 0 }
  }
};
...
function createTableRows(data) {
  ...
  data.forEach(row => {
    ...
    const recordTypeClass = row.record_type ? `row-${row.record_type.toLowerCase().replace(/\s+/g, '-')}` : '';
    if (recordTypeClass) {
...
async function fetchClusteredPage(pageNum) {
  ...
  try {
    ...
    state.clustered.summary = d.summary || { Campaign: 0, Inbound: 0, Outbound: 0 };
    ...
    const s = state.clustered.summary;
    elements.stats.innerHTML = `Showing <strong>${startRec.toLocaleString()}-${endRec.toLocaleString()}</strong> of <strong>${d.totalRecords.toLocaleString()}</strong> records &nbsp;|&nbsp; Campaign: ${s.Campaign.toLocaleString()} &nbsp; Inbound: ${s.Inbound.toLocaleString()} &nbsp; Outbound: ${s.Outbound.toLocaleString()} &nbsp;|&nbsp; Page ${d.page} of ${d.totalPages}`;
    elements.stats.classList.remove('is-hidden');
...
async function exportAllAsCSV() {
  ...
  try {
    ...
    const s = state.clustered.summary;
    elements.stats.innerHTML = `Showing <strong>${startRec.toLocaleString()}-${endRec.toLocaleString()}</strong> of <strong>${total.toLocaleString()}</strong> records &nbsp;|&nbsp; Campaign: ${s.Campaign.toLocaleString()} &nbsp; Inbound: ${s.Inbound.toLocaleString()} &nbsp; Outbound: ${s.Outbound.toLocaleString()} &nbsp;|&nbsp; Page ${pg} of ${state.clustered.totalPages} &nbsp; <em>(CSV exported ${allRecords.length.toLocaleString()} records)</em>`;
...
function appendTableRows(tbody, rows, startIndex = 0) {
  ...
  rows.forEach((row, index) => {
    ...
    const columns = [
      ...
      { value: row.phone_number || '', isHTML: false },
...
function generateCSV() {
  // Check if we're exporting BLA Hot Patch Transfer data
...
// Generate CSV specifically for BLA Hot Patch Transfer data
function generateBLACSV() {
  if (!state.currentBLAData || state.currentBLAData.length === 0) {
    showError('No BLA transfer data to export');
    return;
  }
  
  // Define headers for BLA Hot Patch Transfer report - match table headers
  const headers = [
    'S.No.',
    'Call Type',
    '1st Call ID',
  ...
  state.currentBLAData.forEach((record, index) => {
    ...
    const campaign_agent_history_text = agentHistoryToText(record.campaign_agent_history);
    const inbound_agent_history_text = agentHistoryToText(record.inbound_agent_history);
    ...
    const csvRow = [
      ...
      `"${(record.inbound_recording || '').replace(/"/g, '""')}"`,
  ...
  // Generate filename with current date
  const now = luxon.DateTime.now().setZone(TIMEZONE);
  const filename = `bla_hot_patch_transfer_${now.toFormat('yyyy-MM-dd_HHmmss')}.csv`;
  link.setAttribute('download', filename);
  ...
  console.log(`📊 BLA CSV: Exported ${state.currentBLAData.length} transfer records to CSV`);
}
...
function handleSubmit(e) {
  ...
  // Clear BLA data when running regular report
  state.currentBLAData = null;
...
// Handle BLA Hot Patch Transfer button click
async function handleBLAHotPatchTransfer() {
  console.log('🔥 BLA HOT PATCH: Button clicked, starting transfer report...');
  ...
  params.append('start', elements.startInput.value);
  params.append('end', elements.endInput.value);
  
  // Add optional filters
  const agentName = document.getElementById('agent_name')?.value;
  const extension = document.getElementById('extension')?.value;
  const queueCampaignName = document.getElementById('queue_campaign_name')?.value;
  
  if (agentName) params.append('agent_name', agentName);
  if (extension) params.append('extension', extension);
  if (queueCampaignName) params.append('queue_campaign_name', queueCampaignName);
  ...
  try {
    console.log('🔥 BLA HOT PATCH: Fetching transfer report...');
    
    const response = await fetch(`/api/reports/bla-hot-patch-transfer?${params.toString()}`);
    ...
    if (!result.success) {
      throw new Error(result.message || 'Failed to generate BLA Hot Patch Transfer report');
    }
    
    console.log('✅ BLA HOT PATCH: Report generated successfully', result.summary);
    ...
    // Display results
    displayBLAHotPatchTransferResults(result.data, result.summary);
    
    // Enable CSV download and set BLA data for export
    elements.csvBtn.disabled = false;
    state.currentBLAData = result.data; // Store BLA data for CSV export
    
    toggleLoading(false);
    
  } catch (error) {
    console.error('❌ BLA HOT PATCH ERROR:', error);
    showError(`Failed to generate BLA Hot Patch Transfer report: ${error.message}`);
    toggleLoading(false);
  }
}

// Display BLA Hot Patch Transfer results
function displayBLAHotPatchTransferResults(data, summary) {
  if (!Array.isArray(data) || data.length === 0) {
    elements.resultTable.innerHTML = '<tr><td colspan="100%" class="has-text-centered">No BLA Hot Patch Transfer records found</td></tr>';
    elements.stats.innerHTML = 'No transfer records found for the selected criteria.';
    elements.stats.classList.remove('is-hidden');
    return;
  }
  ...
  // Create table headers for BLA Hot Patch Transfer report
  const headers = [
    'S.No.',
    'Call Type',
    '1st Call ID',
  ...
  data.forEach((record, index) => {
    ...
    const call1RecordingButton = record.campaign_recording ? 
      `<button class="button is-small is-info is-rounded play-btn" title="Play Call 1 Recording" data-src="${record.campaign_recording}" data-meta="${record.campaign_recording}/meta">
        <span class="icon is-small"><i class="material-icons">play_arrow</i></span>
      </button>` : '';
    
    const call2RecordingButton = record.inbound_recording ? 
      `<button class="button is-small is-info is-rounded play-btn" title="Play Call 2 Recording" data-src="${record.inbound_recording}" data-meta="${record.inbound_recording}/meta">
        <span class="icon is-small"><i class="material-icons">play_arrow</i></span>
      </button>` : '';
    ...
    tableHTML += `<tr class="row-bla-transfer">
      ...
      <td style="background-color: #ffebee;">${formatDate(record.transfer_time, record.transfer_time_formatted) || ''}</td>
  ...
  // Update stats with color legend
  const statsHTML = `
    <strong>BLA Hot Patch Transfer Report Summary:</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    📊 Total Linked Transfers: <strong>${data.length}</strong><br>
    📈 Campaign-to-Inbound Calls: <strong>${summary?.campaign_to_inbound_links || 0}</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    📈 Outbound-to-Inbound Calls: <strong>${summary?.outbound_to_inbound_links || 0}</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    📈 Inbound-to-Inbound Calls: <strong>${summary?.inbound_to_inbound_links || 0}</strong>
  `;
  ...
  const playButtons = elements.resultTable.querySelectorAll('.play-btn');
...
function init() {
  ...
  elements = {
    ...
    blaHotPatchBtn: document.getElementById('blaHotPatchBtn'),
  ...
  // Add BLA Hot Patch Transfer button listener
  console.log('🔍 Looking for BLA Hot Patch Transfer button...');
  console.log('Button element:', elements.blaHotPatchBtn);
  
  if (elements.blaHotPatchBtn) {
    console.log('✅ BLA Hot Patch Transfer button found, adding event listener');
    elements.blaHotPatchBtn.addEventListener('click', handleBLAHotPatchTransfer);
    
    // Test if button is clickable by adding a simple test
    elements.blaHotPatchBtn.addEventListener('click', function() {
      console.log('🎯 BLA button clicked - event listener working!');
    });
    
    // Make sure button is not disabled
    elements.blaHotPatchBtn.disabled = false;
    console.log('Button disabled status:', elements.blaHotPatchBtn.disabled);
  } else {
    console.error('❌ BLA Hot Patch Transfer button not found in DOM');
  ...
  // Add BLA function to window object for testing
  window.handleBLAHotPatchTransfer = handleBLAHotPatchTransfer;
  window.testBLAButton = function() {
    console.log('🧪 Testing BLA button...');
    const btn = document.getElementById('blaHotPatchBtn');
    console.log('Button found:', btn);
    console.log('Button disabled:', btn ? btn.disabled : 'N/A');
    console.log('Button style:', btn ? btn.style.cssText : 'N/A');
    if (btn) {
      btn.click();
    }
  };
